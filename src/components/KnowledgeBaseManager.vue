...
const props = defineProps({
  pageTitle: {
    type: String,
    default: "Improve Query",
  },
  pageDescription: {
    type: String,
    default: "Lihat riwayat perbaikan query dan saran optimasi dari AI",
  },
  sectionTitle: {
    type: String,
    default: "Riwayat Query Improvement",
  },
  initialCollection: {
    type: String,
    default: "bpr_supra_rag",
  },
  showBackButton: {
    type: Boolean,
    default: true,
...
const retrainSteps = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      desc: pct >= 100 ? "Data selesai disimpan." : "Data Sedang Disimpan...",
...
onMounted(() => {
  fetchCollection(props.initialCollection)
})
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 400 && backendMsg.includes("SQL Ditolak")) {
        userFriendlyMessage = "Silakan perbaiki query Anda."
      } else if (res.status === 400 && backendMsg.includes("kosong")) {
        userFriendlyMessage = "Mohon lengkapi Prompt dan SQL sebelum menyimpan."
      } else if (res.status === 500) {
        userFriendlyMessage = "Terjadi masalah saat memproses data. Silakan coba lagi beberapa saat lagi."
      } else {
    ...
    if (
      ...
      userMessage = !navigator.onLine ? "Tidak ada koneksi internet. Pastikan perangkat Anda terhubung." : "Tidak dapat menghubungi server."
    } else {
  ...
  if (activeCollection.value === "bpr_supra_cache") {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        let userFriendlyMessage = ""
        if (res.status === 400 && backendMsg.includes("SQL Ditolak")) {
          userFriendlyMessage = "Update dibatalkan. Silakan ganti query Anda."
        } else if (res.status === 400) {
          userFriendlyMessage = `Gagal Update: ${backendMsg}`
        } else if (res.status === 500) {
          userFriendlyMessage = "Terjadi masalah saat memproses data. Silakan coba lagi nanti."
        } else {
      ...
      error.value = String(err)
      let finalMessage = (!navigator.onLine) ? "Tidak ada koneksi internet." : err.message
...
async function confirmDelete() {
  ...
  const collectionLabel = activeCollection.value === "bpr_supra_rag" ? "RAG Collection" : "Cache"

  try {
    const res = await fetch(`${API_BASE_URL}/admin/qdrant/delete`, {
      method: "DELETE",
      headers: getAuthHeaders({ "Content-Type": "application/json" }),
      credentials: "include",
      body: JSON.stringify(payload),
    ...
    showToast(`Data berhasil dihapus dari ${collectionLabel}!`, "success")
    showDeleteModal.value = false
  } catch (err) {
    console.error("Delete error", err)
    let userMsg = (!navigator.onLine)
      ? "Tidak ada koneksi internet. Pastikan perangkat Anda terhubung."
      : (err.message || "Gagal menghapus data.")
    showToast(`Gagal menghapus: ${userMsg}`, "error")
  } finally {
    ...
    <header class="sticky-header">
      <div class="header-inner">
        ...
        <div class="header-title">
          <h1 class="page-title">
            <Icon icon="solar:magic-stick-3-bold-duotone" class="title-icon" />
            {{ pageTitle }}
          </h1>
                ...
                <div class="retrain-checklist-card">
                  ...
                  <div class="retrain-header">
                    ...
                    <div class="header-text">
                      <h3 class="retrain-title">Pelatihan & Sinkronisasi RAG System</h3>
                  ...
                  <div class="retrain-progress-box">
                    <div class="progress-info-row">
                      <span class="progress-status-label">
                        ...
                        {{ progressPercentage < 100 ? 'Memproses Data Vektor...' : 'Pelatihan Selesai!' }}
                      </span>
...
<style scoped src="../views/ImproveQuery/style.css"></style>
